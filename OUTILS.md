...
| **`SOCLE/agents/mini-jeux-projet.md` toujours présent, et son retrait est bloqué** | Ses pièges sont versés dans `AGENTS.md`, donc le retrait ne perdrait plus rien — mais ProlexCore n'a pas pu jouer le `git rm`, et la commande est chez Matt. Le lien vit toujours : `ls -l ~/.claude/agents/mini-jeux-projet.md` | Rien à ma main. **`AGENTS.md` fait foi** en attendant : ne pas compter sur la disparition du fichier |
| ~~La garde A1 refuse les identifiants que D151 demande~~ | **Tranché par D164**, dans le sens recommandé d'ici : la liste reste close, un motif `claude-<depot>-*` rendrait la garde permissive là où elle protège | L'identifiant s'écrit dans `local.json` **du même geste que le worktree** et s'en retire avec lui — porté à `CLAUDE.md` §Déléguer |
